...
import com.example.libraryapp.View.Adapter.OnShopItemClickListener
...
class Fragment_compras : Fragment(), OnShopItemClickListener {

    lateinit var recyclerShop: RecyclerView
    lateinit var adapter: ComprarAdapter
    val viewmodel by lazy { ViewModelProvider(this).get(ComprasViewModel::class.java) }
    ...
    override fun onCreateView(
        ...
        adapter=ComprarAdapter(requireContext(),this)
    ...
    private fun makePurchase() {
        ...
        builder.setPositiveButton("Ok"){
                dialog,which->
            findNavController().navigate(R.id.action_fragment_compras_to_fragment_menu)
        }
    ...
    override fun onItemClickDelete(book: compras, position: Int) {
        database.collection("Compra")
            .document(book.Title)
            .delete()
    }
